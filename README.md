...
6. 儀表板最上方是**本月預估卡片**：預估課程數量（含已排定但還沒上的未來課程）、預估課程收入、每堂場地費（可直接編輯覆蓋當月數字，不填則沿用設定頁的預設值；場地費總額 = 每堂金額 × 本月堂數，不用自己算）、扣掉場地費後的淨收入。
7. 下面是每位學生一張卡片，顯示：
...
「記帳」頁面是獨立於課程堂數/收款追蹤之外的個人收支記錄：

- **教學收入自動帶入**：只要學生的課程包在「學生管理」被標記為已收款，這筆錢就會自動以「教學收入（自動）」出現在記帳的月份總覽裡（用收款日期入帳），不用手動再記一次；要修正金額或收款日期，回「學生管理」改原始購買紀錄即可，記帳頁面本身不能編輯這幾筆。
- **其他收支手動輸入**：生活開銷分「生活」「興趣/裝備」「教練業務」三組分類（見 `lib/types.ts` 的 `EXPENSE_CATEGORIES`），收入則先只有一個「其他收入」分類 + 備註欄位——如果之後想細分自媒體/業配/滑雪教學等來源、看收入結構佔比，再回來擴充分類即可。
- 頁面上方可切換月份，顯示當月總收入、總支出、淨結餘，以及各分類小計。
- 資料一樣存在 `lib/kvStore.ts` 那份 KV（`expense:transactions` 這個 key），不需要額外的環境變數或設定。
...
app/
  ...
  expenses/page.tsx         記帳（生活開銷收支，含自動帶入的教學收入）
  ...
  api/
    ...
    transactions/              記帳交易 CRUD + 月度收支總覽（含自動帶入的教學收入）
    settings/                 讀寫提醒門檻/日曆ID/每堂場地費預設值/上課顏色篩選
    dashboard/                核心數據計算結果 + 本月預估（含事件/顏色篩選診斷數字）
    venue-fee/                 設定當月每堂場地費覆蓋值
    sync/                     手動同步（當月）
    cron/sync/                 排程自動同步（Vercel Cron 呼叫，當月）
lib/
  kvStore.ts                 KV 讀寫抽象層（雲端用 Upstash Redis，本機開發 fallback 成 JSON 檔）
  store.ts                   學生/購買/月份快照/設定/Token/記帳交易的型別化存取
  google.ts                  Google OAuth + 日曆事件讀取
  matching.ts                 姓名比對邏輯
  sync.ts                     同步主流程：整月重新抓取並覆蓋快照
  metrics.ts                   每位學生核心數據計算（FIFO 分包）+ 本月預估（含未來已排定課程）
  expenseMetrics.ts            記帳：教學收入自動帶入 + 月度收支總覽計算
